...
export async function serve({
  ...
  app.post("/api/:key/open-windows-wsl-path", async (req, res) => {
    if (!isSameOriginRequest(req, allowedHostnames, allowAnyHostname)) {
      res.status(403).json({ error: "cross-origin Windows path request rejected" });
      return;
    }
    ...
    try {
      const child = spawn("explorer.exe", [windowsPath], { detached: true, stdio: "ignore" });
      child.unref();
      res.json({ ok: true });
    } catch {
      res.status(503).json({ error: "Windows Explorer is unavailable" });
    }
  });
  ...
  app.get("/api/:key/history", async (req, res, next) => {
    try {
      const session = await store.findByKey(req.params.key);
      if (!session) {
        res.status(404).json({ error: "session not found" });
  ...
  app.post("/api/:key/history/:version/restore", async (req, res, next) => {
    try {
      const session = await store.findByKey(req.params.key);
      if (!session) {
        res.status(404).json({ error: "session not found" });
      ...
      await writeFile(session.file, snap.html, "utf8");
      res.json({ restored: true, version: snap.version });
    } catch (error) {
      next(error);
    }
  });
  ...
  app.get("/session/:key", async (req, res, next) => {
    try {
      ...
      const initialAnnotate = resolveInitialAnnotateMode(req.query || {}, process.env);
      res.type("html").send(
...
export function resolveInitialAnnotateMode(query = {}, env = process.env) {
  const queryVal = String(query.annotate || query.mode || "")
    .trim()
    .toLowerCase();
  if (queryVal === "off" || queryVal === "false" || queryVal === "0" || queryVal === "explore") {
    return false;
  }
  if (queryVal === "on" || queryVal === "true" || queryVal === "1" || queryVal === "annotate") {
    return true;
  }
  if (
  ...
  const envVal = String(env.LAVISH_AXI_ANNOTATE_DEFAULT || "")
    .trim()
    .toLowerCase();
...
export function createChromeHtml(
  ...
  const sessionJson = jsonScript({
    ...
    initialAnnotate: Boolean(initialAnnotate),
    chromeLoadToken,
    layoutGateEnabled,
