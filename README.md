...
- **Human-AI collaboration** - Annotate elements and selected text ranges, edit Mermaid whiteboard diagrams, and send messages to the agent without leaving Lavish Editor. Lavish opens reviews in explore mode by default, so ordinary left clicks remain ordinary browser interactions and the right click opens annotation; use `?annotate=on` when left-click annotation is explicitly wanted. `?annotate=off` remains accepted for links that already use it.
- **Live Web App Proxy** - Review and annotate running local dev servers (`http://localhost:3000`, `http://127.0.0.1:8000`) or web applications directly through a built-in streaming reverse proxy with full duplex forwarding, automatic asset rewriting, storage polyfills, and duplicate session canonicalization.
...
### Reviewing Live Web Applications (`lavish-axi <url>`)

You can wrap running web applications and local development servers into Lavish without generating static HTML files:

```sh
lavish-axi http://127.0.0.1:8000/setup
```

Lavish runs a streaming reverse proxy under `/artifact/<key>/proxy/`, rewrites relative asset paths, injects the Lavish Annotation SDK and storage polyfills, and canonicalizes URLs (`localhost` vs `127.0.0.1`, trailing slashes, parameter order) to prevent duplicate sessions.
In Codex and Antigravity, use the companion skill `/lavish-wrapper <url>` to open and monitor live web apps.
...
- **Export and sharing** - `lavish-axi export` writes `<name>.export.html` by inlining local assets only, stripping the annotation SDK, and leaving remote CDN/font references as links that still need network access. An artifact that declares a relative `<meta name="source-markdown-path" content="report.md">` beside its HTML also exposes its original Markdown in the Editing menu: the full source path can be copied and the bound Markdown can be downloaded. Lavish resolves that reference only within the artifact directory.
...
| `lavish-axi <html-file \| url>`      | Open or resume a Lavish Editor session (for local HTML files or running web applications via reverse proxy), with the open-time layout gate enabled by default. Unresolved layout issues from earlier in the session are preserved. Refuses to reopen a session the user explicitly ended from the browser unless `--reopen` is passed.                   |
| `lavish-axi poll <html-file \| url>` | Long-poll until the user sends feedback, ends the session, or leaves every review disconnected past the reconnect grace period; detected layout issues arrive only when queued. On `browser_disconnected`, ask before reopening or ending the still-resumable session. On `ended`, stop polling and do not reopen uninvited.                              |
| `lavish-axi end <html-file \| url>`  | End a session as the agent; unlike a user-initiated end from the browser, this still allows a plain reopen later.                                                                                                                                                                                                                                         |
...
| ------------------------------- | --------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `lavish-axi <html-file \| url>` | `--no-open`           | Ensure the server/session exists without opening another browser window.                                                                                                                                                            |
| `lavish-axi <html-file \| url>` | `--no-gate`           | Skip the open-time layout curtain for this browser open.                                                                                                                                                                            |
| `lavish-axi <html-file \| url>` | `--reopen`            | Reopen a session the user explicitly ended from the browser; without it, a plain open refuses and explains why instead of reopening uninvited.                                                                                      |
